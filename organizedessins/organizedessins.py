...
drawing_dir = u"C:\\Pieces\\PLANS DE DEFINITION (X__-XXXX)"
srcpath = drawing_dir
destpath = u"C:\\Pieces\\PLANS DE DEFINITION (X__-XXXX)"
...
with open("log.txt", "a") as f:
    for root, subFolders, files in walklevel(srcpath, level=3):
        for file in files:
            base, extension = os.path.splitext(file)
            #print(extension[1:])
            if extension[1:].upper() in ["PDF", "DXF", "STEP", "DWG", "SLDPRT", "SLDDRW", "SLDASM", "JPG", "IGS",
                                         "P2M", "PNG", "AI", "P2D", "STP", "DWL", "DWL2", "BAK", "MCAM"]:
                if foo.match(file):
                    subFolder = os.path.join(destpath, getFamille(file[:3].capitalize()), file[:8])
                    print (subFolder)
                    # print file
                    ...
                    if os.path.isfile(os.path.join(subFolder, file)):
                        ...
                        #result = tkMessageBox.askquestion("Remplacer",
                                                          #(file[:11] + u" existe! Voulez-vous le remplacer?"),
                                                          #icon='warning')
                        #if result == 'yes':
                        ...
                        #base, extension = os.path.splitext(file)
                        i = 1
                        while os.path.exists(os.path.join(subFolder, '{}{}.double_{}'.format(base, extension, i))):
                            i += 1
                        shutil.move(os.path.join(root, file), os.path.join(subFolder, '{}{}.double_{}'.format(base, extension, i)))
                        print("Moved " + file)
                    else:
